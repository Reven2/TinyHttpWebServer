...
void Task::doit()
{
	...
	while(int nread = recv(accp_fd,buf,BUFSIZ,0))
	{
		...
		else if(method == "POST") deal_post(uri,buf);
...
int Task::deal_get(const string &uri)
{
	//从uri第一个字节开始复制字串到filename
	 ...
	 if( uri == "/" || uri == "/index.html" ) {
		send_file( "submit.html", "text/html" );
	 } 
...
{
	string filename = uri.substr(1);
	//寻找cgi程序 sub ，准备参数 传入execl执行
 //	if(uri.find( "sub" ) != string::npos)
//	{
		char *tmp = buf;
		int len,a,b;

		//返回那串字符串下一字符位置，即获得报文实体体大小
		char *ptr = strstr(tmp,"Content-Length:");
		//把报文实体大小 赋值给 len
		sscanf(ptr,"Content-Length: %d",&len);

		//整个请求体大小 - 实体体大小 = 非实体体 大小
		len = strlen(tmp) - len ;
		//加上非实体体大小 即可使从实体体内容地址开始
		tmp += len;
		//把请求实体体内 a ,b 值分别存入定义的 a ,b 变量
...
//	else cout<<"404 Not found CGI sub"<<endl;
}
